...
One of the best (somewhat accidental) innovations of the haven package is the introduction of labelled vectors and other metadata tags into R that we commonly see when working with other statistical software.
...
We'll be going through some brief examples of working with labels, but for a more detailed general introduction see the [Introduction to labelled](https://larmarange.github.io/labelled/articles/intro_labelled.html) vignette.
...
Labelled datasets are great for accessing metadata in the R console, but there are a few other useful applications for data analysis, tabling and processing.

### Factors and character conversion

The labelled package has a set of helper functions for converting labelled vectors to factors and character vectors.
...
Labels that don't exist in the data can be dropped from the levels.
...
Factor Levels can easily be sorted by either value or label. By default, they are sorted by value.
...
Unlabelled values can be discarded.
...
The `to_character()` function allows you to convert to a character vector instead of a factor, using the same general conversion rules
